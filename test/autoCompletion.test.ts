...
describe('Auto Completion Tests', () => {
  ...
  describe.only('Completion duplicity description', () => {
    it('Should add description for duplicity items that have data.schemaTitle', async () => {
      const schema = {
        anyOf: [
          {
            title: 'Schema1',
          ...
          },
          {
            ...
            type: 'object',
            properties: {
              prop1: { type: 'object', default: { prop12: {} } },
            },
          },
          {
            // no title defined
            type: 'object',
            properties: {
              prop1: { type: 'object', default: { prop13: {} } },
            },
          },
        ],
      };
      ...
      const content = '';
      const result = await parseSetup(content, content.length);
      // TODO: test it's not finished because label vs labelDetails problem
      expect(result.items.map((i) => ({ label: i.label, data: i.data, labelDetails: i.labelDetails }))).to.deep.equal([
        {
          label: 'prop1',
          data: {
            schemaTitle: 'Schema1',
